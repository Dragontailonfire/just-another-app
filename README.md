...
│   ├── HTMLExportService.swift        # Netscape HTML bookmark export (browser-importable)
│   │
│   └── Assets.xcassets/               # Asset catalog (includes light/dark/tinted app icons)
│
...
│
├── generate_app_icon.swift             # Standalone Swift script to generate app icon PNGs
...
- Custom bookmark icon with light (blue-to-indigo gradient), dark (near-black), and tinted (gray) variants.
- Generated by `generate_app_icon.swift` — standalone Swift script using CoreGraphics (`swift generate_app_icon.swift`).
- Outputs 1024×1024 PNGs to `Assets.xcassets/AppIcon.appiconset/`.
...
- Each folder detail view has a search bar that filters both bookmarks and subfolders in real time.
- Filters by bookmark name, URL, and description; subfolders by name.

### HTML Export (`HTMLExportService.swift`)

- Exports all bookmarks as a Netscape Bookmark File Format HTML file.
- Preserves folder hierarchy as nested `<DL>` sections.
- Importable in Safari (File > Import), Chrome, Firefox, and Edge.
- Available in Settings > Data > "Export as HTML" via ShareLink.
